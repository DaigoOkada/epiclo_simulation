#snalysis (2023.5.1)
#source("/Users/dokada/Dropbox/analysis/2022.5/methy_ra_drift0518.R") #Run 5.18
set.seed(1000)
library(igraph)
library(glmnet)
library(parallel)
out_path <- "/Users/dokada/Desktop/work/methy_ra_drift0518/"
...
Methy_data_simu_drift <- function(n_markers=100, ages=20:80, n_tissues=c(100, 100, 100, 100), n_sel_cell=10, n_cores=9, 
    ...
    wrapper_simu <- function(k){
        ...
        tmp_acc <- dam_speed[k]
        celltypes <- c(rep(1, 20), rep(2, 20), rep(3,20)) #ここが違う
        n_cells <- length(celltypes)
        ...
        for(j in 1:n_ages){
            tmp_dat <- dat_onehuman[,,j]
            weight_cs1 <- 40 + j * w_a1
            weight_cs2 <- 20 + j * w_a2
            weight_cs3 <- 20 + j * w_a3
            sample1_idx <- sample(which(celltypes==1), weight_cs1, replace=T)
            sample2_idx <- sample(which(celltypes==2), weight_cs2, replace=T)
            sample3_idx <- sample(which(celltypes==3), weight_cs3, replace=T)
            tmp_dat_sample <- tmp_dat[c(sample1_idx, sample2_idx, sample3_idx),]
            bulk[j,] <- colMeans(tmp_dat_sample) + as.integer(round(rnorm(n_markers, 0, measure_noise)))
        }
...
#Subsetdrift
set.seed(1100)
out_dir <- paste0(out_path, "drift_com2/")
...
out_file_list <- list()
w_can <- c(rep(0, 50), rep(2, 50))
n_rep <- length(w_can)
seeds_can <- 1:n_rep
for(i in 1:length(seeds_can)){
    myseed <- seeds_can[i]*2
    subset_prop1 <- lapply(1:4, function(x){c(1/3, 1/3, 1/3)})
    out_file <- paste0(out_dir, "para", ".ds", i, ".rds")
    #out_file_list[[i]] <- Methy_data_simu2(n_markers=100, myseed=myseed, NSS=0.2, r_com=0.2, out_file=out_file,  cellsubsetdrift=TRUE, subset_prop=subset_prop1, w_a1=w_can[i], w_a2=0, w_a3=0)
    out_file_list[[i]] <- Methy_data_simu_drift(n_markers=100, myseed=myseed, NSS=0.5, r_com=0.5, out_file=out_file, w_a1=w_can[i], w_a2=0, w_a3=0, alpha=10)
    cat(i, "\n")
}
res <- Epiclo_est2(out_file_list, out_dir)
write.csv(w_can, file=paste0(out_dir, "w_can.csv"))